...
enum class Currency {
    ...
    companion object {
        fun of(value: String): Currency {
            if (!values().map { currency -> currency.name }.contains(value)) {
                throw CurrencyValidationException("Currently available currencies: ${values().joinToString(",")}")
            } else {
...
class CurrencyValidationException(message: String) : RuntimeException(message)
